...
import { isGyaanEditorEnabled } from '@/lib/config/feature-flags';
...
export function EditChromeRoot({ scene, isEditable, onToggleEditMode }: EditChromeRootProps) {
  ...
  useEffect(() => {
    document.body.dataset.gyaanEditor = 'true';
    return () => {
      delete document.body.dataset.gyaanEditor;
    };
  }, []);
    ...
    <EditShell
      ...
      commandTrailing={
        <HeaderControls
          mode="edit"
          canEdit={isEditable}
          onToggleEditMode={isGyaanEditorEnabled() ? onToggleEditMode : undefined}
        />
      }
    />
  );
