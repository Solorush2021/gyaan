...
export function CommandBar({ title, history, commands, trailing }: CommandBarProps) {
  ...
  return (
    <header className="flex h-20 shrink-0 items-center gap-3 border-b border-border/60 px-8 bg-card">
      <div className="flex min-w-0 flex-1 items-center gap-2">
        ...
        <span
          className={cn('ml-2 truncate text-sm font-semibold text-foreground')}
          title={title}
        >
          {title}
        </span>
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <Button
          size="icon-sm"
          variant="ghost"
          className="h-8 w-8 shrink-0 rounded-xl text-muted-foreground hover:bg-accent hover:text-accent-foreground"
          {...props}
        >
          {children}
        </Button>
