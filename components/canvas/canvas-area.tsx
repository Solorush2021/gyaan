...
export function CanvasArea({
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[var(--gyaan-slate)] dark:bg-[var(--gyaan-slate)] group/canvas">
      ...
      <div
        className={cn(
          ...
          currentScene?.type === 'interactive'
            ? 'bg-[var(--gyaan-teal)]/5 dark:bg-[var(--gyaan-teal)]/8'
            : 'bg-[var(--gyaan-slate)] dark:bg-[var(--gyaan-slate)]',
        )}
      >
        <div
          className={cn(
            'aspect-[16/9] h-full max-h-full max-w-full bg-background dark:bg-card shadow-2xl rounded-lg overflow-hidden relative transition-all duration-700',
            showControls && !isLiveSession && currentScene?.type === 'slide' && 'cursor-pointer',
            currentScene?.type === 'interactive'
              ? 'shadow-[0_8px_40px_var(--gyaan-glow)] ring-1 ring-[var(--gyaan-teal)]/15'
              : 'shadow-[0_8px_32px_oklch(0_0_0/0.3)] ring-1 ring-white/5',
          )}
          ...
          <AnimatePresence>
            ...
            {isPendingScene && !currentScene && !isCourseComplete && (
              <motion.div
                ...
                className="absolute inset-0 z-[105] flex flex-col items-center justify-center bg-background dark:bg-card">
                ...
                {isGenerationFailed ? (
                  ...
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="relative w-12 h-12">
                      <div className="absolute inset-0 rounded-full border-2 border-muted dark:border-muted" />
                      <div className="absolute inset-0 rounded-full border-2 border-transparent border-t-[var(--gyaan-teal)] dark:border-t-[var(--gyaan-teal-light)] animate-spin" />
                    </div>
                    {/* Text */}
                    <motion.span
              ...
              >
                ...
                >
                  <motion.div
                    ...
                    className="w-20 h-20 rounded-full bg-background/95 dark:bg-card/95 flex items-center justify-center shadow-[0_4px_30px_var(--gyaan-glow),inset_0_0_0_1px_color-mix(in_oklch,var(--gyaan-teal)_20%,transparent)]"
                    style={{ willChange: 'transform' }}
                  >
                    <Play className="w-7 h-7 text-[var(--gyaan-teal)] dark:text-[var(--gyaan-teal-light)] fill-[var(--gyaan-teal)]/90 dark:fill-[var(--gyaan-teal-light)]/90 ml-0.5" />
                  </motion.div>
                </motion.div>
              </motion.div>
            )}
      ...
      {!hideToolbar && (
        <CanvasToolbar
          className={cn(
            ...
            'border-t border-[var(--gyaan-teal)]/15 dark:border-[var(--gyaan-teal)]/10',
          )}
