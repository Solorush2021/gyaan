    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="h-[85vh] p-0 gap-0 block" showCloseButton={false}>
        ...
        <div className="flex h-full overflow-hidden">
          ...
          <div
            className="flex-shrink-0 bg-[#F5EDE0]/30 dark:bg-neutral-900/30 backdrop-blur-md p-3.5 flex flex-col gap-3.5 border-r border-[#133F25]/10 dark:border-white/5 overflow-y-auto"
            style={{ width: sidebarWidth }}
          >
            {/* AI Services */}
            <div className="space-y-1">
              <div className="px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-[#133F25]/50 dark:text-emerald-400/50">
                {t('settings.aiServices') || 'AI Services'}
              </div>

              <button
                onClick={() => setActiveSection('providers')}
                className={cn(
                  'relative w-full flex items-center gap-3 px-3 py-2.5 text-sm rounded-lg transition-all duration-300 text-left min-w-0 border outline-none font-medium',
                  activeSection === 'providers'
                    ? 'bg-[#133F25]/10 border-[#133F25]/20 text-[#133F25] dark:bg-emerald-400/10 dark:border-emerald-400/20 dark:text-emerald-300 shadow-sm before:absolute before:left-0 before:top-2.5 before:bottom-2.5 before:w-1 before:rounded-r before:bg-[#133F25] dark:before:bg-emerald-400'
                    : 'bg-white/10 border-white/5 dark:bg-neutral-800/10 dark:border-white/5 hover:bg-[#133F25]/5 hover:border-[#133F25]/10 dark:hover:bg-emerald-400/5 dark:hover:border-emerald-400/10 text-muted-foreground hover:text-[#133F25] dark:hover:text-emerald-300 hover:shadow-sm',
                )}
              >
            ...
            {/* Separator */}
            <div className="h-px bg-[#133F25]/10 dark:bg-white/5 my-1" />

            {/* Features & Content */}
            <div className="space-y-1">
              <div className="px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-[#133F25]/50 dark:text-emerald-400/50">
                {t('settings.featuresContent') || 'Features & Data'}
              </div>
