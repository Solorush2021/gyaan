...
export function HeaderControls({
  ...
  return (
    <div className="flex items-center gap-4">
      <div
        className={cn(
          'shrink-0 flex items-center gap-1 backdrop-blur-md shadow-sm rounded-full bg-(--glass-bg) border border-(--border-subtle)',
      ...
      >
        ...
        <DropdownMenu modal={false}>
          <DropdownMenuTrigger asChild>
            <button
              className="p-2 rounded-full text-(--muted-foreground) hover:bg-(--secondary) hover:text-(--foreground) hover:shadow-sm transition-all group"
          ...
          <DropdownMenuContent align="end" sideOffset={8} className="min-w-[140px] bg-(--popover) text-(--popover-foreground) border border-(--border-subtle) shadow-(--shadow-premium) rounded-lg overflow-hidden">
            <DropdownMenuItem
              onSelect={() => setTheme('light')}
              className={cn(
                'cursor-pointer gap-2 hover:bg-(--secondary)',
                theme === 'light' &&
      ...
      <div className="relative" ref={exportRef}>
        ...
        {exportMenuOpen && (
          <div className="absolute top-full mt-2 right-0 bg-(--popover) border border-(--border-subtle) rounded-lg shadow-(--shadow-overlay) overflow-hidden z-50 min-w-[200px]">
            <button
              onClick={() => {
                setExportMenuOpen(false);
                exportPPTX();
              }}
              className="w-full px-4 py-2.5 text-left text-sm hover:bg-(--secondary) hover:text-(--foreground) transition-colors flex items-center gap-2.5"
            >
