...
/**
 * Gyaan Editor (Pro mode) gate. Default OFF — gates only the Pro toggle
 ...
 * state.
 */
export function isGyaanEditorEnabled(): boolean {
  return readBoolean(process.env.NEXT_PUBLIC_GYAAN_EDITOR_ENABLED);
}
...
export function isVocationalTaskEngineEnabled(): boolean {
  return readBoolean(process.env.GYAAN_ENABLE_VOCATIONAL);
}
