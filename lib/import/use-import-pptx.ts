...
// pdfjs-dist (transitively pulled by `gyaan-importer/src`) uses dynamic
...
import type * as GyaanImport from '@gyaan/importer';

const log = createLogger('ImportPptx');

export type PptxUpload = NonNullable<GyaanImport.ImportPptxOptions['upload']>;
 ...
 * the bundled `gyaan-importer` dist that we load by URL to bypass
  ...
  const handleFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        // `scripts/sync-gyaan-importer.mjs` copies the prebuilt dist into
        // `public/vendor/` after every `pnpm install`.
        const url = '/vendor/gyaan-importer/index.js';
