...
import { resolveSlide, isGridSlide } from './grid-layout';
import { postProcessInteractiveHtml } from './interactive-post-processor';
import { parseActionsFromStructuredOutput } from './action-parser';
import { parseJsonResponse } from './json-repair';

/**
 * [C] Retry wrapper for small local models that frequently emit malformed JSON.
 * Re-prompts up to `maxRetries` times with a "your output was invalid" nudge.
 * Only retries when the response fails to parse as JSON.
 */
async function aiCallWithJsonRetry(
  aiCall: AICallFn,
  systemPrompt: string,
  userPrompt: string,
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    const response = await aiCall(systemPrompt, currentUser, images);
    lastResponse = response;
    const parsed = parseJsonResponse(response);
    if (parsed !== null) {
      return response; // valid JSON — ship it
    }
    if (attempt < maxRetries) {
      log.warn(`LLM returned malformed JSON (attempt ${attempt + 1}/${maxRetries + 1}), retrying...`);
      currentUser =
        userPrompt +
        '\n\n[IMPORTANT: Your previous output was NOT valid JSON. Output ONLY a single valid JSON object. No prose, no code fences, no trailing commas, fully closed brackets.]';
    }
  }
  log.warn('aiCallWithJsonRetry exhausted retries, returning last response for best-effort parsing');
  return lastResponse;
}
import {
  buildCourseContext,
...
async function generateSlideContent(
  ...
  // GRID-DSL MODE: use the trimmed mini prompt + resolve layout in code.
  // Gated behind env flag so the original cloud path stays untouched.
  const useGridDsl = process.env.GYAAN_GRID_DSL === '1' || process.env.GYAAN_GRID_DSL === 'true';
  if (useGridDsl) {
    const miniPrompts = buildPrompt(PROMPT_IDS.SLIDE_CONTENT_MINI, {
      title: outline.title,
    ...
    const miniResponse = await aiCallWithJsonRetry(aiCall, miniPrompts.system, miniPrompts.user, visionImages);
    const gridData = parseJsonResponse<unknown>(miniResponse);
    if (gridData && isGridSlide(gridData)) {
      const resolved = resolveSlide(gridData, outline.order ?? 0);
      // Grid engine returns PPTElement[] directly — feed into the same
      // post-processing pipeline (fixElementDefaults → latex → image resolve)
      // as the legacy path by constructing a GeneratedSlideData-shaped object.
      const gridResult = {
        elements: resolved.elements as unknown as GeneratedSlideData['elements'],
        background: resolved.background as GeneratedSlideData['background'],
      };
      // Re-run the same downstream pipeline as the legacy path so ids,
      // latex, and image ids are all normalized.
      const fixedElements = fixElementDefaults(gridResult.elements, assignedImages);
      ...
      const videoNormalizedElements = normalizeGeneratedVideoRefs(resolvedElements);
      const processedElements: PPTElement[] = videoNormalizedElements.map((el) => ({
        ...el,
        // [SPOTLIGHT] PRESERVE role-based IDs from grid-layout (e.g. 'title',
        // 'bullet_0', 'card_1'). The mini prompt tells the model to emit
        // spotlight actions targeting these role-IDs, so they MUST survive
        // intact. Only assign a nanoid fallback when no role-ID was set.
        id: el.id || `${el.type}_${nanoid(8)}`,
        rotate: 0,
      })) as PPTElement[];
      log.debug(`Grid-DSL produced ${processedElements.length} elements for: ${outline.title}`);
      // [A] MERGE: carry the model's actions onto the content so
      // generateSceneActions can reuse them and skip its 2nd LLM call.
      const gridActions = (gridData as { actions?: unknown[] }).actions;
      return {
        elements: processedElements,
        background: gridResult.background,
        actions: Array.isArray(gridActions) ? gridActions : undefined,
      };
    }
    // If grid parsing failed, fall through to the legacy path below.
    log.warn('Grid-DSL parse failed, falling back to legacy slide-content prompt');
  }
  ...
  const response = await aiCallWithJsonRetry(aiCall, prompts.system, prompts.user, visionImages);
...
export async function generateSceneActions(
  ...
  if (outline.type === 'slide' && 'elements' in content) {
    // [A] MERGE: if content already carries actions (grid-DSL merged call),
    // skip the second LLM call entirely and use them directly.
    const mergedActions = (content as GeneratedSlideContent & { actions?: unknown[] }).actions;
    if (Array.isArray(mergedActions) && mergedActions.length > 0) {
      // Convert the merged wire-format actions into valid Action objects.
      // The mini prompt emits {type:'text', content:'...'} for speech segments;
      // 'text' is the intermediate wire format, NOT a valid Action type.
      // Map it to the real 'speech' action (field is `text`, not `content`).
      // Non-speech action types from the model (spotlight/play_video/etc.) are
      // passed through with their original fields so they survive the merge.
      // Spotlight elementIds target role-based IDs (e.g. 'bullet_0') that are
      // preserved on the elements — validate they exist, drop if not.
      const validElementIds = new Set(content.elements.map((e) => e.id));
      const rawActions: Action[] = mergedActions
        .filter((a): a is Record<string, unknown> => !!a && typeof a === 'object')
        .map((a) => {
          const aType = typeof a.type === 'string' ? a.type : 'text';
          if (aType === 'text') {
            // speech wire-format → real speech action
            const speechText =
              typeof a.content === 'string' ? a.content
              : typeof a.text === 'string' ? a.text
              : String(a.content ?? a.text ?? '');
            return { type: 'speech', text: speechText } as unknown as Action;
          }
          // pass through real action types verbatim (spotlight, play_video, etc.)
          return { ...a } as unknown as Action;
        })
        .filter((a) => {
          if (a.type === 'speech') return Boolean((a as { text?: string }).text);
          // spotlight/laser must reference a real element id; drop orphans
          if (a.type === 'spotlight' || a.type === 'laser') {
            const eid = (a as { elementId?: string }).elementId;
            return !!eid && validElementIds.has(eid);
          }
          return true;
        });
      if (rawActions.length > 0) {
        log.debug(`[Merge] Reusing ${rawActions.length} merged actions, skipping 2nd LLM call`);
        return processActions(rawActions, content.elements, agents);
      }
    }
