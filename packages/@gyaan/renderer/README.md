...
React component for rendering PPTist-style `Slide` JSON. Extracted from [Gyaan](https://github.com/THU-GYAAN/Gyaan).
...
```bash
pnpm add @gyaan/renderer
# or
npm install @gyaan/renderer
```
...
```tsx
import { SlideCanvas, type Slide } from '@gyaan/renderer';
...
```tsx
import { SlideRendererProvider, SlideCanvas, useSlideContext } from '@gyaan/renderer';
...
### Granular components — `@gyaan/renderer/elements`
...
export default {
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@gyaan/renderer/dist/**/*.{js,cjs}',
  ],
};
```
...
```ts
import '@gyaan/renderer/fonts.css';
```

> **Runtime dependency — read this.** The `@font-face` `src` URLs point at an
> external font host (`https://file.gyaan.chat/fonts/<name>.woff2`); the woff2
...
[`@gyaan/importer`](../importer) converts `.pptx` files to the same `Slide[]` shape, so you can do `.pptx → @gyaan/renderer` end-to-end.
