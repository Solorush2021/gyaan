 ...
 *
 * The app loads the parser at runtime from `/vendor/gyaan-importer/index.js` — a
 * gitignored build artifact that `scripts/sync-gyaan-importer.mjs` copies into
...
const entry = path.join(root, 'public/vendor/gyaan-importer/index.js');
...
try {
  ...
  console.error('[assert-vendor] It is loaded at runtime via /vendor/gyaan-importer/index.js');
  console.error('[assert-vendor] and is produced by the postinstall sync step. To fix:');
  console.error(
    '[assert-vendor]   pnpm --filter @gyaan/importer build && pnpm run sync:gyaan-importer',
  );
